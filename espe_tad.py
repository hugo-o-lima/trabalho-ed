...
class Colecao:
    ...
    def __init__(self):
        ...
        >>> x = Colecao()
        '''
        self.figurinhas = array(10, Figurinha(0))
        self.figurinhas_repetidas = array(10, 0)
    ...
    def gera_figurinhas_repetidas(self) -> str:
        ...
        for i in range(len(self.figurinhas)):
            if self.figurinhas_repetidas[i] != 0:
                aux += str(self.figurinhas[i].numero) + " (" + str(self.figurinhas_repetidas[i]) + "), "
        aux = aux[:-2]
        aux += "]"
        return aux 
